...
const SwapTokens = ({
  ...
  getTokenAddress,
  error,
  krestPrice // Add krestPrice as a prop
}) => {
  ...
  useEffect(() => {
    const calculateMinimumAmountOut = async () => {
      if (amountA !== '' && amountA >= 0.1 && slippage >= 0 && tokenA && tokenB && signer) {
        try {
          ...
          let tokenAddressA = getTokenAddress(tokenA);
          let tokenAddressB = getTokenAddress(tokenB);

          if (tokenA === 'KRST') {
            tokenAddressA = '0xDd11f4E48CE3A2B9043B2B0758ce704d3Fd191dc'; // WKREST address
          }
          if (tokenB === 'KRST') {
            tokenAddressB = '0xDd11f4E48CE3A2B9043B2B0758ce704d3Fd191dc'; // WKREST address
          }
          ...
          const minReceived = parseFloat(ethers.utils.formatUnits(calculatedAmountOutMin, decimalsB));
          const minReceivedUSD = (minReceived * krestPrice).toFixed(8);
          setMinReceivedUSD(minReceivedUSD);
        } catch (error) {
  ...
  }, [amountA, slippage, tokenA, tokenB, signer, krestPrice]);
  ...
  return (
    <>
      ...
      <GreyedOutUSD>
        Minimum Received: ~${minReceivedUSD}
      </GreyedOutUSD>
      {renderButton()}
      <ExchangeRate>
